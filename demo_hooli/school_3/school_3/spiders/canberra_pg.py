...
class PlymouthSpider(CrawlSpider):
    ...
    start_urls = ['http://search.canberra.edu.au/s/search.html?collection=courses&form=course-search&profile=_default&query=!padre&course-search-widget__submit=&meta_C_and=COURSE&sort=metaH&f.Type|B=postgraduate&start_rank=1']
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'http://search.canberra.edu.au/s/search.html?collection=courses&form=course-search&profile=_default&query=!padre&course-search-widget__submit=&meta_C_and=COURSE&sort=metaH&f.Type|B=postgraduate&start_rank=\d+'),follow=True),
